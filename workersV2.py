            ...
            for term, cats in self.lex.items():
                try:
                    ...
                    for cat, weight in cats.items():
                        try:
                            if cat == "affect":
                                pLex[cat] += float(1)*float(weight)
                            else:
                                pLex[cat] += float(gn)*float(weight)
                        except KeyError:
                            if cat == "affect":
                                pLex[cat] = float(1)*float(weight)
                            else:
...
if __name__ == '__main__':
    samples = [str (
        ...
        str('Something </sarcasm> about <fails to break this up> <3 </3 <\\3 mañana vergüenza güenza création tonterías tonteréas <em class="grumpy">pain</em> <meta name="viewport" content="width=device-width"> <br />' + " " +
    ...
    tok = LexiconExtractor("topics2000")
    results = tok.get_scores(samples)
    for item in results:
        print(item)